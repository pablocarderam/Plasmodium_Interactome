 ...
 * Main renderer call
 */
function render(data_file) {
  sigma.parsers.json(

    // First we import the data from a json file:

    data_file, // first argument is data filepath
    ...
    function(s) {
      ...
      function searchGene(e) {
        ...
        var matches = []; // track all possible matches
        ...
        s.graph.nodes().forEach(function(n) { // loop over all nodes
          if (n.id === _.$('gene_id').value) { // if exact match found,
            ...
            matches.push(n.id); // add to matches
          }
        ...
        if (close_match.length < 1) { // if no matches were found at all,
          ...
          if (matches.length > 1) {
            alert('Multiple nodes containing '+_.$('gene_id').value+' were found:'+ matches.join('\n')+'\n\n'+match+' will be selected by default. To select any others, search for the exact above IDs.');
          }
        }
      }
      ...
      /**
       * Set what the dataset in UI should display
       */
      ...
      _.$('dataset').addEventListener("change",setDataset);
    }
  );
}

function restart() {
  _.$("sigma-container").remove();
  _.$('sigma-container-outer').innerHTML = '<div id="sigma-container"></div>';
